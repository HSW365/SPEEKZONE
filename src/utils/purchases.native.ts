 ...
 *
 *   1. App Store Connect → one auto-renewable subscription must be created
 *      with this exact product ID (already referenced in
 *      src/utils/data.ts):
 *        com.speekzone.app.verified_monthly   ($9.99/mo)
 ...
 *      entitlement called "verified" attached to the product above, add it
 *      to the app's current offering, and copy the public iOS SDK key into
 *      REVENUECAT_IOS_API_KEY below.
 *
...
function planFromCustomerInfo(info: CustomerInfo): 'free' | 'verified' {
  return info.entitlements.active[ENTITLEMENT_ID] ? 'verified' : 'free';
}

export async function getCurrentPlan(): Promise<'free' | 'verified' | null> {
 ...
 */
export async function purchasePlan(appleProductId: string): Promise<'free' | 'verified'> {
  if (!configured) {
